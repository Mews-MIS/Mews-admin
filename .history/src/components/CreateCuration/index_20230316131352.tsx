...
const CreateCuration = () => {
  ...
  useEffect(() => {
    const getArticles = async () => {
      const response = await CurationAPI.getPageArticles({ page });
      await setAllCurationList(response!.articles);
    };
    getArticles();
  }, [page]);
  //  (async() => {
          ...
          <div className="w-1/2">
            ...
            <div className="h-4/5 w-4/5  overflow-y-scroll border border-gray-500">
              <div className="flex-col flex-nowrap w-full">
                <div className="flex-col flex-nowrap w-full">
                  {allCurationList &&
                    allCurationList.map((data) => {
                      return (
                        <div key={data.id} className="font-bold border border-gray-500 w-full h-8">
                          {data.title}
                        </div>
                      );
                    })}
