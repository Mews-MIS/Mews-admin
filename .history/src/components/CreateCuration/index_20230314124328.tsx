...
const CreateCuration = () => {
  ...
  // 모든 기사들 불러오기
  const [allCurationList, setAllCurationList] = useState<AllCuration[] | null>([]);
  const [isChecked, setIsChecked] = useState<boolean>(false);
  const [allCuration, setAllCuration] = useState<AllCuration | null>(null);

  useEffect(() => {
    CurationAPI.getCurationAll().then((data) => {
      setAllCurationList(data.allCuration);
      console.log(data.allCuration, "dddd");
      console.log("test", allCurationList);
    });
  }, []);
          ...
          <div className="w-1/2">
            ...
            <div className="h-4/5 w-4/5  overflow-y-scroll border border-gray-500">
              <div className="flex-col flex-nowrap w-full">
                {/* {allCurationList
                  ? allCurationList.map((data, index) => (
                      <div
                        key={index}
                      ...
                      >
                        <div className="flex flex-row my-auto">
                          <button
                            ...
                            // onClick={() => data.id != null && isChecked(data.id)}
                          >
                            X
                          </button>
                          <div className="my-auto text-lg">{data.title}</div>
                        </div>
                      </div>
                    ))
                  : ""} */}
