      ...
      <div className="w-full h-full pl-8 pt-8">
        <form
          onSubmit={handleSubmit(async (data) => {
            try {
              ...
              formData.append(
                "data",
                new Blob([JSON.stringify(form)], { type: "applicaiton/json" })
              );
              console.log(JSON.stringify(form));
              const response = await CurationAPI.postCreateCuration(formData);
            } catch (e) {
              console.log(e);
            }
          })}
        >
          ...
          <input
            type="text"
            id="title"
            {...register("title", { required: "큐레이션 제목을 입력해주세요." })}
